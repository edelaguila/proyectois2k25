    ...
    public class Sentencias
    {
        ...
        public OdbcDataAdapter queryProv(string sQuery)
        {
            string sql = "SELECT Pk_id_deuda, Fk_id_proveedor, Efecto_trans, Fk_id_factura, " +
                         ...
                         "FROM " + sTabla_Proveedor + " " +
                         "WHERE CAST(Fk_id_proveedor AS CHAR) LIKE '%" + sQuery + "%';";

            OdbcDataAdapter dataTable = new OdbcDataAdapter(sql, conexion.conexion());
            return dataTable;
        }

        string sTabla_Cliente = "Tbl_Deudas_Clientes";
        ...
        public OdbcDataAdapter queryCliente(string sQuery)
        {
            string sql = "SELECT Pk_id_deuda, Fk_id_cliente, Fk_id_cobrador, Fk_id_factura, " +
                         ...
                         "FROM " + sTabla_Cliente + " " +
                         "WHERE CAST(Fk_id_cliente AS CHAR) LIKE '%" + sQuery + "%';";
